...
document.addEventListener("DOMContentLoaded", (event) => {
  ...
  // 부자재
  const subsidiarySelector = document.getElementById('subsidiary-select__selector')
  const subsidiarySize = document.getElementById('subsidiary-size')
  const subsidiaryCountInput = document.getElementById('sign-subsidiary-cnt-select')
  const subsidiarySizeSelect = document.getElementById('sign-subsidiary-size-select')
  const addSubsidiaryButton = document.getElementById('add-subsidiary-button')

  // 부자재 선택 시 이미지 업데이트 이벤트 리스너 등록
  document.getElementById('subsidiary-select__selector').addEventListener('change', updateSubsidiaryImage);
  ...
  woodAddButton.addEventListener('click', e => {
    ...
    const processingType = processingSelect.value === '0' ? '경첩홈파기' : '우라홈(+2000)'
    ...
    if(!lengthSelect.value || !widthSelect.value || !signColorSelect.value || !document.getElementById('sign-thickness-select').value || !signCountInput.value )
    {
      alert('모든 항목을 입력해주세요.');
      return;
    }
    ...
    */
    ...
    const detail = `${woodType || ''} / 두께 ${thickness || ''} / 폭 ${width || ''} / 길이 ${length|| ''} / ${count || '0'}개 / ${processingType} / ${formattedPrice} 원`
  ...
  addSubsidiaryButton.addEventListener('click', () => {
    ...
    if (subsidiaryValue <= 1) price = smartBarPriceTable[subsidiaryValue][subsidiarySizeSelect.value];
    ...
    const detail = `${subsidiaryType || ''} / 길이 ${subsidiarySize || ''} / ${subsidiaryCnt || '0'}개 / ${formattedPrice}원`
  ...
  function updatePrice() {
    ...
    if(lengthIndex === -1 || widthIndex === -1 || processingIndex === -1) return
